...
- 上部モードは `Move`、`Shape`、`Connect` です。`Shape`では選択球体に大きなXYZスケールハンドルが表示されます。`Connect`では球体を2つ順に選ぶと関係線を即時作成し、右側で関係ラベル・関係種別・比較次元を編集できます。2つ目以降を`Shift` / `Ctrl`（Macは`Cmd`）を押しながら選ぶと、連続して関係線を作れます。線をクリックすると既存の関係を再編集できます。
