...
- 位置は直接ドラッグとObject EditorのXYZ数値入力で編集できます。形はObject EditorのScale X/Y/ZとBase sizeで確実に調整できます。
- 空白部分のドラッグでカメラ回転、ホイールでズーム、右ドラッグでパンします。
- XYZの軸線は原点から正・負の両方向へ同じ長さで表示されます。
- 画面上部の `3D / XY / YZ / XZ` で、立体表示と各座標平面への正対ビューを切り替えられます。選択中の平面でも通常の編集操作を継続できます。
...
- 操作モードは使い分け不要です。球体ドラッグはMove、選択球体の周囲に現れる赤X・緑Y・青ZハンドルはShape、`Shift + click`で2球を順に選択するとConnectionを作成します。ShiftドラッグはZ方向Moveとして判定され、Connectionにはなりません。関係線の中点ラベルはダブルクリックで直接編集でき、`Enter`またはフォーカスアウトで保存、`Esc`でキャンセルします。線またはラベルの選択後、`Delete` / `Backspace`で削除できます。
- Viewバーの `↶ Undo` または `Ctrl + Z`（macOSは `Cmd + Z`）で、そのブラウザで行った直近のView編集を最大50件まで取り消せます。テキスト入力欄を編集している間の標準Undoは妨げません。
