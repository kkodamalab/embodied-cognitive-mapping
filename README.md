...
- 操作モードは使い分け不要です。球体ドラッグはMove、ShapeはObject EditorのScale X/Y/Z、Connectionは`Ctrl + click`（Macは`Cmd + click`）で2球を順に選択します。2球目を選ぶと関係線を即時作成し、右側で編集できます。線をクリックすると既存の関係を再編集できます。
